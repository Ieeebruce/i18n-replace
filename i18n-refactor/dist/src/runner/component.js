...
function buildAliases(code) {
    ...
    const raw = (0, var_alias_1.collectVarAliases)(sf, 'locale', 'getLocale'); // 通过 AST 收集别名
    const out = []; // 输出列表
...
function replaceTs(src) {
    ...
    const composeKey = (ai, path) => {
        if (ai.prefix) { // 前缀别名：this.<alias>=getLocale().x.y
            ...
            if (path.startsWith(rootFirst + '.'))
                path = path.slice(rootFirst.length + 1); // 去重根段
            return `${ai.prefix}.${path}`; // 拼接前缀
        }
        if (ai.roots && ai.roots.length) { // 合并来源别名：按 roots 选根
            const r = (0, dict_reader_1.pickRoot)(ai.roots, path); // 动态选根
        ...
        if (ai.name === 'i18n') { // 普通 i18n 别名：尝试按常见根选择
            ...
            if (candidates.includes(seg0))
                return path; // 已含根则原样返回
            const r = (0, dict_reader_1.pickRoot)(candidates, path); // 动态选根
    ...
    for (const ai of aliases) { // 遍历别名进行替换
        ...
        s = s.replace(new RegExp(`this\\.${name}\\.([A-Za-z0-9_.]+)((?:\\.replace\\([^)]*\\))+)`, 'g'), (_m, path, chain) => {
            const params = (0, params_extractor_1.extractReplaceParams)(chain); // 提取参数
            return (0, ts_replace_1.renderTsGet)(name, { keyExpr: composeKey(ai, String(path)), params }); // 渲染 get 调用
        });
        s = s.replace(new RegExp(`this\\.${name}\\.([A-Za-z0-9_.]+)\\s*\\[\\s*'([^']+)'\\s*\\]`, 'g'), (_m, base, lit) => {
            const path = `${String(base)}.${String(lit)}`; // 拼接路径
            return (0, ts_replace_1.renderTsGet)(name, { keyExpr: composeKey(ai, path) }); // 渲染
        });
        s = s.replace(new RegExp(`this\\.${name}\\.([A-Za-z0-9_.]+)\\s*\\[\\s*\"([^\"]+)\"\\s*\\]`, 'g'), (_m, base, lit) => {
            const path = `${String(base)}.${String(lit)}`; // 拼接路径
            return (0, ts_replace_1.renderTsGet)(name, { keyExpr: composeKey(ai, path) }); // 渲染
        ...
        s = s.replace(new RegExp(`this\\.${name}\\.([A-Za-z0-9_.]+)\\s*\\[([^\\]]+)\\]`, 'g'), (_m, base, expr) => {
            ...
            return (0, ts_replace_1.renderTsGet)(name, { keyExpr: `'${basePath}.' + ${String(expr).trim()}` }); // 拼接表达式
        });
        s = s.replace(new RegExp(`this\\.${name}\\.(?!get\\b)([A-Za-z0-9_.]+)(?!\\s*\\(|\\s*\\.replace|\\s*\\[|\\s*=)`, 'g'), (_m, path) => {
            return (0, ts_replace_1.renderTsGet)(name, { keyExpr: composeKey(ai, String(path)) }); // 渲染
        });
    }
    return s; // 返回替换后的代码
}
function replaceHtml(src, aliases) {
    ...
    s = s.replace(/\{\{\s*([A-Za-z_]\w*)\.([A-Za-z0-9_.]+)((?:\.replace\([^)]*\))+)[^}]*\}\}/g, (_m, v, key, chain) => {
        ...
        if (!ai)
            return _m; // 未识别则原样返回
        const rp = ai.roots && ai.roots.length ? (0, dict_reader_1.pickRoot)(ai.roots, String(key)) : ''; // 选根
    ...
    s = s.replace(/\{\{\s*([A-Za-z_]\w*)\.([A-Za-z0-9_.]+)\s*\[(['"])([^'\"]+)\3\]\s*\}\}/g, (_m, v, base, _q, lit) => {
        ...
        if (!ai)
            return _m; // 未识别返回
        const rp = ai.roots && ai.roots.length ? (0, dict_reader_1.pickRoot)(ai.roots, String(base)) : ''; // 选根
        const rootPrefix = rp ? rp + '.' : ''; // 根前缀
