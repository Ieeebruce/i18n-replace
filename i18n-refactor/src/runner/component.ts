...
import { pickRoot, getAllRoots } from '../util/dict-reader' // 导入字典根选择工具
...
export function processComponent(tsCode: string, htmlCode: string, filePath?: string): { tsOut: string, htmlOut: string } { // 编排组件：TS 与 HTML 一致替换
  ...
  for (const n of varNames) { tsOut = tsOut.replace(new RegExp(`\\b${n}\\s*:\\s*any\\s*;`, 'g'), '') } // 额外移除残留声明
  const assigned = Array.from(new Set((tsOut.match(/this\.([A-Za-z_]\w*)\s*=\s*/g) || []).map(s => s.replace(/^this\./, '').replace(/\s*=.*/, ''))))
  for (const n of assigned) { tsOut = tsOut.replace(new RegExp(`\\b${n}\\s*:\\s*any\\s*;`, 'g'), '') }
