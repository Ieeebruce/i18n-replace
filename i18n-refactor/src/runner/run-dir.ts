...
function checkAndInstallNgxTranslate(mode: 'report'|'fix') {
  ...
  if (!hasNgxTranslate) {
    if (mode === 'fix') {
      ...
      warn('missing @ngx-translate/core package', { suggest: 'npm install @ngx-translate/core' });
    }
  } else {
...
function createNgxTranslateService(dictDir: string, mode: 'report'|'fix') {
  ...
  if (!fs.existsSync(coreDir)) {
    if (mode === 'fix') {
      ...
      warn('core directory missing', { suggest: 'create src/app/core' });
      return;
    }
  }
...
`;
      ...
      warn('missing i18n service', { suggest: 'create src/app/core/i18n.service.ts' });
    }
  } else {
...
function modifyAppModule(mode: 'report'|'fix') {
...
export function injectNgxTranslate(dictDir: string, mode: 'report'|'fix') {
...
export function main() {
  ...
  let mode: 'replace' | 'restore' | 'bootstrap' | 'delete' | 'init' | 'dict-process' | 'inject-i18n' = 'replace'
  ...
  if (mode === 'inject-i18n') {
    injectNgxTranslate(config.dictDir || 'src/app/i18n', (config.ensureAngular || 'fix'))
    return
  }
