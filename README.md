#请智能分析并修改app.config.ts文件：

1. 分析现有结构：
   - 检查是否已导入@ngx-translate/core
   - 检查providers配置是否存在
   - 检查是否已有TranslateModule配置

2. 按需添加配置：
   - 仅在缺失时添加必要导入
   - 在providers数组中添加TranslateModule配置
   - 确保HttpLoaderFactory函数存在
...
4. 配置内容：
   - 使用importProvidersFrom包装TranslateModule
   - 配置loader使用HttpLoaderFactory
   - 确保依赖项正确注入
## i18n-refactor 使用教程
